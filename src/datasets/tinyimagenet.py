...
def download_and_unzip(URL, root):
  if not os.path.exists(root+'tiny-imagenet-200/'):
...
Args:
  root: Where the data is located
...
class TinyImageNetPaths:
  def __init__(self, root, download=True):
    ...
    val_path = os.path.join(root, 'tiny-imagenet-200', 'val')
    test_path = os.path.join(root, 'tiny-imagenet-200', 'test')

    wnids_path = os.path.join(root, 'tiny-imagenet-200', 'wnids.txt')
    words_path = os.path.join(root, 'tiny-imagenet-200', 'words.txt')
